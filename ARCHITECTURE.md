...
| --- | --- | --- |
| Desktop shell | Electron, context isolation, Chromium sandbox, `webSecurity`, no renderer Node integration/`webview`, trusted sender checks, and strict navigation/external-origin allowlists | Installer signing, update channel, clean-machine and release smoke tests |
| Renderer | React + TypeScript + Vite; canonical-state reload after main-process changes; modal dialog semantics/focus trap/focus restore and reduced-motion CSS | End-to-end coverage, screen-reader/keyboard accessibility, localization, and performance review |
| Local data | Main-process SQLite with WAL, foreign keys, transactions, legacy JSON import, strict invariants, sync metadata/conflicts | Crash injection, clean-shutdown/recovery audit, migration-upgrade and fuzz/concurrency testing |
...
| Timer | Timestamp + pause-interval model; one active timer; recovery UI; bounded lease guard/renewal when cloud is available | Real multi-device, offline-overlap, and network-fault validation |
...
| Cloud sync | Empty-device cloud bootstrap, durable outbox, pull-before-push cursor sync, 10-second RPC deadlines, conservative conflict records/UI and explicit local/cloud choices, claimed-account eligibility, SQL RPCs and RLS migrations | Apply migrations, real RLS/multi-device integration tests, conflict UX policy, monitoring, backup/export |
| Currency and analytics | Original-currency facts; validated FX cache; stored-timezone allocation; matching-currency rate denominator | Production FX-provider contract/reliability/privacy review and real-service tests |
...
The main process serializes renderer mutations, overlay actions, explicit account operations, and timer-lease requests through one queue. For a timer start/resume, a read-only `CommandService.preflight()` checks the strict schema and current durable transition before any asynchronous lease call. That prevents an invalid or stale timer command from reserving a cloud lease, and keeps the preflight result valid until the queued command commits.
...
- Before first local setup, an online-authenticated device with no account calls the self-scoped cloud snapshot RPC. A found workspace is atomically restored locally without an outbox echo. Before an existing local account is claimed, the main process repeats that snapshot check and refuses the claim if the cloud account already has a workspace. An offline or failed check cannot fall through to create/claim a potentially overwriting cloud profile.
...
The repository contains no credentials or hosted Supabase project. The OAuth callback checks the exact `timefarm://auth/callback` route, a pending TimeFarm-generated nonce, and the PKCE flow ID before exchange; the encrypted continuation expires after ten minutes and is consumed before the network exchange. A full callback threat model, error telemetry, secure-storage failure handling, and real provider verification remain release gates.

## Cloud schema, sync, and conflicts

[0001_workly_schema.sql](supabase/migrations/0001_workly_schema.sql) and [0002_bootstrap_snapshot.sql](supabase/migrations/0002_bootstrap_snapshot.sql) are **unapplied migration artifacts**. Together they create profiles, projects, sessions, pauses, payments, goals, sync operation/change records, FX cache rows, timer lease rows, a self-scoped bootstrap snapshot RPC, and canonical profile/preferences payloads. RLS and RPC code derive ownership from `auth.uid()` rather than trusting a client-supplied user ID.
...
For a brand-new device, `workly_bootstrap_snapshot` runs before any onboarding-created profile or normal outbox exists. If it finds a cloud profile, the repository atomically imports the returned completed history and preference state, clears generated outbox rows, marks imported entities synced, and saves the snapshot cursor. If no profile is found, onboarding remains local and an explicit later claim is still protected by a second online snapshot check.
...
1. Pull changes through `workly_pull_changes` from the durable account cursor, in pages of up to 100 changes (at most 50 pages per run). A pull error or 10-second RPC deadline is a safety stop: no outbox operation is pushed during that run, and a late page is inert.
...
3. Push up to 50 ready outbox operations through `workly_apply_sync_operation`, retaining failed operations and scheduling retry. The same 10-second deadline applies to push and bootstrap RPCs; a late success cannot acknowledge or mutate a later run.

Remote sync never drives a running or paused timer. If remote data would overwrite a divergent pending local entity, an active local timer, protected history, or another unsafe deletion, the repository retains local data/outbox work and records a durable conflict with both payloads and the remote cursor. The renderer offers **Keep local & retry**, which acknowledges the conflict while retaining the pending local operation, or **Use cloud version**, which atomically applies the recorded remote payload only after the same local safety checks and cancels the losing pending operation. There is no field-level merge, duplicate-overlap resolver, cloud monitoring, or deployed-service proof yet.
...
- Missing configuration, missing authentication, or an RPC/network failure clears any local claim and is **not** treated as a held lease. The local offline timer still functions, but overlapping offline work must be reviewed later; the lease is not an offline consensus mechanism.
...
- An effective hourly rate uses the active duration of only the sessions with earnings in that same currency. Foreign work time therefore cannot lower or inflate that original-currency rate.
...
- `electron/fx-service.cjs` validates a Frankfurter-compatible provider response, caches it atomically, exposes fresh/stale/unavailable status, and never converts without a verified cached pair. Its production provider contract, privacy, reliability, and rate-quality work remains open.

## Build, CI, and distribution

Electron is pinned to `43.3.0`, matching the Node 24 runtime used by the SQLite repository. The GitHub Actions workflow at `.github/workflows/ci.yml` runs on Windows with Node 24 and performs frozen install, lint, tests, and renderer build. It does not publish or sign a release.
...
Packaging includes the compiled renderer and main/overlay files and relies on Electron's bundled `node:sqlite`, not a native SQLite addon. A clean-machine installer pass, code signing, malware/reputation review, upgrade testing, privacy/support materials, auto-update strategy, and release publication remain explicit gates.
