...
This document separates code present in this repository from production proof. "Implemented" means the local code path exists and has its listed automated coverage. Production migrations and unauthenticated RPC denial boundaries have also been hosted-verified; real-provider desktop login, authenticated data round-trips, and physical multi-device behavior are separately identified where they remain unverified.
...
| Authentication         | Supabase email/password and Google entry points; encrypted session plus cached identity; 1.5-second bounded remote verification; provider-derived account binding; exact production provider and desktop callback configuration                                   | Installed-app PKCE/provider integration, restart/error-path, and telemetry verification            |
| Cloud sync             | Empty-device cloud bootstrap, durable outbox, pull-before-push cursor sync, per-entity optimistic revisions/CAS, 10-second RPC deadlines, conservative conflict records/UI, atomic claims, migrations `0001`–`0008`, fresh pgTAP replay, and hosted denial probes | Authenticated round-trips, physical multi-device tests, monitoring, and backup/export              |
...
`SupabaseAuthService` is disabled unless the desktop process receives valid `TIMEFARM_SUPABASE_URL` and `TIMEFARM_SUPABASE_ANON_KEY` values (`WORKLY_*` and `VITE_*` names are compatibility fallbacks). The variable name is retained for compatibility, but the runtime accepts a Supabase publishable key or a legacy `role=anon` JWT and rejects secret/service-role/malformed keys. Production uses a dedicated publishable desktop key with legacy anon/service-role API-key headers disabled.
...
The repository contains no credentials; hosted configuration is supplied from ignored local files or protected release secrets. Google is configured External / In production with the exact Supabase provider callback, while Supabase permits only `timefarm://auth/callback?timefarm_state=**` for the desktop return. The app checks that exact route, a pending TimeFarm-generated nonce, and the PKCE flow ID before exchange; the encrypted continuation expires after ten minutes and is consumed before the network exchange. Installed-app provider login, restart/error flows, telemetry, and secure-storage failure handling remain release gates.

## Cloud schema, sync, and conflicts

The ordered files in [`supabase/migrations`](supabase/migrations), currently `0001` through `0008`, are the versioned cloud schema source. Together they create profiles, projects, sessions, pauses, payments, goals, sync operation/change records, durable entity-version tombstones, FX cache rows, timer lease rows, self-scoped bootstrap/revision RPCs, an atomic workspace-claim RPC, canonical profile/preferences payloads, production hardening, retention controls, and repaired timer-lease privileges. RLS and RPC code derive ownership from `auth.uid()` rather than trusting a client-supplied user ID. CI replays the complete chain into disposable Postgres and runs 105 pgTAP assertions; the protected production workflow has deployed through `0008`, and the hosted contract passes all six RPC existence/unauthenticated-denial probes.

The migration chain rejects a non-object sync payload and mismatched entity/body IDs for projects, work sessions, payments, and goals; requires a profile root before any non-account cloud entity can be created; forbids deletion of completed work-session history; blocks deletion of projects with work/payment history; restricts an existing completed-session amendment to the latest completed session; validates every completed-session pause is an ordered, non-overlapping interval within session bounds and that its declared active duration matches timestamps minus pauses; and revokes default `PUBLIC` execution for the RPCs before granting them to `authenticated`. The client also stops its run after a failed root-account operation instead of attempting dependent writes. The migration and unauthenticated-denial boundaries are hosted-verified; authenticated behavior and concurrent-client races remain release gates.
...
The protocol has service and integration-seam tests, and its hosted RPC exists with anonymous execution denied. Authenticated lease acquisition/renewal and competing physical devices are not yet proven. Treat those validations as required before describing it as a production multi-device timer guarantee.
