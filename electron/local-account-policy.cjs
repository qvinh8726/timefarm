...
async function resolveLocalAccountPrincipal({ account, authService } = {}) {
  ...
  if (!eligibility.eligible) {
    return { account: account ?? null, linked: false, offline: false, auth: null, state: eligibility.state }
  }
  if (!authService || typeof authService.getStatus !== 'function') {
    throw new LocalAccountPrincipalError('AUTH_STATUS_UNAVAILABLE', 'Authentication status is unavailable for linked local data.')
  }
  const auth = await authService.getStatus()
  if (!auth?.authenticated || !auth.user?.id || auth.user.id !== account.authUserId) {
    throw new LocalAccountPrincipalError('LOCAL_ACCOUNT_OWNER_MISMATCH', 'The signed-in account does not own this local TimeFarm data.')
  }
  return { account, linked: true, offline: Boolean(auth.offline), auth, state: auth.offline ? 'owned_offline' : 'owned' }
}

module.exports = { LocalAccountPrincipalError, cloudSyncEligibility, resolveLocalAccountPrincipal }
