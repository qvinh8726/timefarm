...
const GOAL_KINDS = ['hours_daily', 'hours_weekly', 'earnings_daily', 'earnings_weekly', 'earnings_monthly', 'projects_completed']
const DASHBOARD_WIDGETS = ['timer', 'goals', 'earningsTrend', 'hoursTrend', 'projectBreakdown', 'rateTrend', 'cumulativeEarnings', 'comparison']
...
const IdentifierSchema = z.string().trim().min(1).max(160).refine((value) => !/[\u0000-\u001f\u007f]/.test(value), {
  message: 'Identifier must not contain control characters.',
})

const IsoTimestampSchema = z.string().trim().datetime({ offset: true }).transform((value) => new Date(value).toISOString())
const TimeZoneSchema = z.string().trim().min(1).max(100).refine(isValidTimeZone, { message: 'Timezone must be a valid IANA timezone.' })
...
const CountrySchema = z.string().trim().regex(/^[A-Za-z]{2,3}$/).transform((value) => value.toUpperCase())
const AuthUserIdSchema = IdentifierSchema

const WidgetListSchema = z.array(z.enum(DASHBOARD_WIDGETS)).max(DASHBOARD_WIDGETS.length).superRefine((value, context) => {
  if (new Set(value).size !== value.length) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Widgets must not contain duplicates.' })
})
// `partialRecord` is intentional: a saved layout only needs to carry sizes
// for widgets whose default size was overridden.
const WidgetSizeSchema = z.partialRecord(z.enum(DASHBOARD_WIDGETS), z.enum(DASHBOARD_WIDGET_SIZES))
...
const ProjectUpdatePayloadSchema = z.object({
  ...
  if (Object.keys(value).length === 1) context.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one project field must be supplied.' })
})
...
const PreferencesPatchSchema = z.object({
  ...
  if (Object.keys(value).length === 0) context.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one preference field must be supplied.' })
})

const command = (type, payload) => z.object({ type: z.literal(type), payload }).strict()

const CommandSchema = z.discriminatedUnion('type', [
  command('account.initialize', z.object({
  ...
  command('account.update-profile', z.object({
    displayName: DisplayNameSchema.optional(),
    language: z.enum(['vi', 'en']).optional(),
    timezone: TimeZoneSchema.optional(),
  }).strict().superRefine((value, context) => {
    if (Object.keys(value).length === 0) context.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one profile field must be supplied.' })
  })),
  command('project.create', ProjectCreatePayloadSchema),
  command('project.update', ProjectUpdatePayloadSchema),
  command('project.set-status', z.object({ projectId: IdentifierSchema, status: z.enum(PROJECT_STATUSES) }).strict()),
  command('project.delete', z.object({ projectId: IdentifierSchema }).strict()),
  command('session.start', z.object({ projectId: IdentifierSchema.optional() }).strict()),
  command('session.pause', z.object({}).strict()),
  command('session.resume', z.object({}).strict()),
  command('session.complete', z.object({
  ...
  command('session.recover-complete', z.object({
    sessionId: IdentifierSchema,
    money: MoneySchema,
    note: OptionalTextSchema.nullable().optional(),
    endedAt: IsoTimestampSchema,
  }).strict()),
  command('session.discard', z.object({ sessionId: IdentifierSchema }).strict()),
  command('session.edit-latest', z.object({
    sessionId: IdentifierSchema,
    money: MoneySchema,
    note: OptionalTextSchema.nullable().optional(),
  }).strict()),
  command('payment.create', z.object({
    projectId: IdentifierSchema,
    money: MoneySchema,
  ...
  command('payment.update', z.object({
    ...
    if (Object.keys(value).length === 1) context.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one payment field must be supplied.' })
  })),
  command('payment.delete', z.object({ paymentId: IdentifierSchema }).strict()),
  command('goal.create', z.object({
    kind: z.enum(GOAL_KINDS),
    target: z.number().finite().positive().max(Number.MAX_SAFE_INTEGER),
  }).strict()),
  command('goal.update', z.object({
    goalId: IdentifierSchema,
    kind: z.enum(GOAL_KINDS).optional(),
    target: z.number().finite().positive().max(Number.MAX_SAFE_INTEGER).optional(),
  }).strict().superRefine((value, context) => {
    if (Object.keys(value).length === 1) context.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one goal field must be supplied.' })
  })),
