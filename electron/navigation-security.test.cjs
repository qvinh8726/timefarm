...
test("only permits exact local renderer origins and packaged files in the dist tree", () => {
  assert.equal(
    isAllowedAppNavigation("http://127.0.0.1:5173/projects?tab=active", {
      isDev: true,
  ...
  assert.equal(
    isAllowedAppNavigation("http://127.0.0.1:5173.evil.example/", {
      isDev: true,
      devServerUrl: "http://127.0.0.1:5173",
    }),
    false,
  ...
  assert.equal(
    isAllowedAppNavigation("http://attacker@127.0.0.1:5173/", {
      isDev: true,
      devServerUrl: "http://127.0.0.1:5173",
    }),
    false,
  ...
  assert.equal(
    isAllowedAppNavigation(
      pathToFileURL(path.join(distDirectory, "assets", "app.js")).toString(),
      {
        isDev: false,
        distDirectory,
      },
  ...
  assert.equal(
    isAllowedAppNavigation(
      pathToFileURL(path.join(distDirectory, "..", "outside.html")).toString(),
      {
        isDev: false,
        distDirectory,
      },
...
test("recognises only the exact TimeFarm OAuth callback from command-line arguments", () => {
  assert.equal(
    normaliseOAuthCallbackUrl(
      "timefarm://auth/callback?code=abc&timefarm_state=expected",
    ),
    "timefarm://auth/callback?code=abc&timefarm_state=expected",
  );
  assert.equal(
    normaliseOAuthCallbackUrl("timefarm://auth.evil.example/callback?code=abc"),
    null,
  );
  assert.equal(
    normaliseOAuthCallbackUrl("timefarm://auth/other?code=abc"),
    null,
  );
  assert.equal(
    normaliseOAuthCallbackUrl("timefarm://auth:443/callback?code=abc"),
    null,
  );
  assert.equal(
    findOAuthCallbackUrl([
  ...
  assert.equal(
    findOAuthCallbackUrl([
      "electron.exe",
      "https://example.com/timefarm://auth/callback",
    ]),
    null,
  );
});
...
test("overlay navigation must remain on its exact generated data URL", () => {
  ...
  assert.equal(
    isAllowedOverlayNavigation("https://evil.example/", overlayUrl),
    false,
  );
});
