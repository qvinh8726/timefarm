...
const { OverlayManager, activeDurationAt, buildTimerSnapshot, clampPosition } = require('./overlay-manager.cjs')
...
class FakeWindow extends EventEmitter {
  ...
  setVisibleOnAllWorkspaces(...value) { this.visibleOnAllWorkspaces = value }
  ...
  setPosition(x, y) { this.bounds.x = x; this.bounds.y = y; this.emit('moved') }
...
function activeState(mode = 'interactive') {
  return {
    ...
    sessions: [{ id: 'session-1', projectId: 'project-1', status: 'running', startedAt: '2026-08-10T00:00:00.000Z', pauses: [] }],
  }
}
...
test('builds an active timer snapshot with pauses excluded', () => {
  ...
  const session = {
    ...
    pauses: [{ startedAt: '2026-08-10T00:03:00.000Z', endedAt: '2026-08-10T00:05:00.000Z' }],
  }
  ...
  assert.deepEqual(snapshot, { status: 'running', projectName: 'Writing', activeDurationMs: 8 * 60 * 1000, tickedAt: now })
})

test('view-only overlay is transparent, click-through, and rejects actions', async () => {
  FakeWindow.instances = []
  const manager = new OverlayManager({ BrowserWindow: FakeWindow, screen: fakeScreen(), preloadPath: '/overlay-preload.cjs' })
  ...
  assert.equal(overlay.webContents.sent.at(-1).channel, 'workly:overlay-snapshot')
})

test('interactive overlay sends timer actions and persists a clamped local position', async () => {
  FakeWindow.instances = []
  const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'workly-overlay-test-'))
  ...
  try {
    const manager = new OverlayManager({
      ...
      positionFilePath,
      onAction: async (action) => { actions.push(action); return { ok: true } },
    })
    ...
    assert.deepEqual(JSON.parse(fs.readFileSync(positionFilePath, 'utf8')), { x: 214, y: 0 })
...
test('clamps a saved position onto the current display work area', () => {
  assert.deepEqual(clampPosition({ x: -4, y: 800 }, fakeScreen(500, 300)), { x: 0, y: 188 })
})
