...
function addTimeFarmOAuthState(redirectUrl, state) {
  ...
  if (parsed.protocol !== 'timefarm:' || parsed.hostname !== 'auth' || parsed.port || parsed.pathname !== '/callback' || parsed.username || parsed.password || parsed.hash) {
...
function normalizePendingOAuth(value, now = Date.now()) {
  if (!value || typeof value !== 'object' || value.version !== OAUTH_PENDING_VERSION) return null
  if (!isValidOAuthState(value.state) || !isValidPkceFlowId(value.flowId)) return null
  if (!Number.isSafeInteger(value.expiresAt) || value.expiresAt <= now) return null
  if (!value.pkce || typeof value.pkce !== 'object' || Array.isArray(value.pkce)) return null
  ...
  if (entries.length === 0 || entries.some(([key, stored]) => !allowedKeys.has(key) || typeof stored !== 'string')) return null
  ...
  if (legacyVerifier !== undefined && parseStoredPkceVerifier(legacyVerifier) !== verifier) return null

  const storedIndex = value.pkce[oauthVerifierIndexStorageKey()]
  if (storedIndex !== undefined) {
    try {
      const index = JSON.parse(storedIndex)
      if (!Array.isArray(index) || index.length !== 1 || index[0] !== value.flowId) return null
    } catch {
      return null
    }
  }
...
function readBundledConfiguration() {
  const candidates = [
    ...
    process.resourcesPath ? path.join(process.resourcesPath, 'timefarm.config.json') : null,
  ].filter(Boolean)
  for (const candidate of candidates) {
    try {
      const value = JSON.parse(fs.readFileSync(candidate, 'utf8'))
      if (value && typeof value === 'object' && !Array.isArray(value)) return value
    } catch { /* optional bundled configuration */ }
  }
  return {}
}
...
function readConfiguration(environment = process.env) {
  ...
  const url = environment.TIMEFARM_SUPABASE_URL || environment.WORKLY_SUPABASE_URL || environment.VITE_SUPABASE_URL || bundled.supabaseUrl || ''
  const anonKey = environment.TIMEFARM_SUPABASE_ANON_KEY || environment.WORKLY_SUPABASE_ANON_KEY || environment.VITE_SUPABASE_ANON_KEY || bundled.supabaseAnonKey || ''
  const redirectUrl = environment.TIMEFARM_OAUTH_REDIRECT_URL || environment.WORKLY_OAUTH_REDIRECT_URL || bundled.oauthRedirectUrl || 'timefarm://auth/callback'
  try {
    const parsed = new URL(url)
    if (parsed.protocol !== 'https:' && parsed.hostname !== 'localhost' && parsed.hostname !== '127.0.0.1') return { configured: false }
  } catch {
...
function normalizeStoredSession(value) {
  ...
  if (value.version === 1 && value.session && typeof value.session === 'object') {
    return { session: value.session, user: sanitizeUser(value.user) ?? sanitizeUser(value.session.user) }
  }
...
function isDefinitiveCredentialError(error) {
  ...
  return /\b(?:invalid|malformed) (?:jwt|token)\b|\b(?:jwt|token) (?:is )?(?:invalid|malformed)\b|\brefresh token (?:is )?(?:invalid|not found|revoked)\b|\bsession (?:not found|revoked)\b/.test(message)
}
...
function offlineStatus(user) {
  return {
    ...
    user: user ?? null,
    offline: true,
    error: 'Unable to verify this session while offline. Local data remains available on this device.',
  }
}

function normaliseHydrationTimeoutMs(value) {
  if (!Number.isInteger(value) || value < 1) throw new TypeError('Auth hydration timeout must be a positive integer number of milliseconds.')
  return value
}
...
class SupabaseAuthService {
  constructor({
    ...
    if (!timeoutScheduler || typeof timeoutScheduler.setTimeout !== 'function' || typeof timeoutScheduler.clearTimeout !== 'function') {
      throw new TypeError('Auth timeoutScheduler must provide setTimeout() and clearTimeout().')
    }
    if (typeof now !== 'function') throw new TypeError('Auth now must be a function.')
    if (typeof randomBytes !== 'function') throw new TypeError('Auth randomBytes must be a function.')
  ...
  createPendingOAuthStorage() {
    return {
      ...
      setItem: async (key, value) => this.setPendingOAuthStorageItem(key, value),
      removeItem: async (key) => this.removePendingOAuthStorageItem(key),
    }
  }

  isPendingOAuthPkceStorageKey(key) {
    if (key === oauthVerifierIndexStorageKey() || key === oauthLegacyVerifierStorageKey()) return true
    return typeof key === 'string' && new RegExp(`^${OAUTH_PKCE_STORAGE_KEY}-flow-[A-Za-z0-9_-]{8,64}-code-verifier$`).test(key)
  }
  ...
  setPendingOAuthStorageItem(key, value) {
    ...
    if (!this.isPendingOAuthPkceStorageKey(key) || typeof value !== 'string') return
  ...
  createOAuthState() {
    ...
    if (!bytes || typeof bytes.length !== 'number' || bytes.length !== 32) {
      throw new Error('Unable to generate a cryptographically secure Google sign-in state.')
    }
    return Buffer.from(bytes).toString('base64url')
  }

  loadPendingOAuth() {
    if (!this.isSecureStorageAvailable() || !fs.existsSync(this.pendingOAuthPath)) return null
    try {
      const encrypted = Buffer.from(fs.readFileSync(this.pendingOAuthPath, 'utf8'), 'base64')
      const pending = normalizePendingOAuth(JSON.parse(this.safeStorage.decryptString(encrypted)), this.now())
      if (!pending) throw new Error('Stored OAuth continuation is malformed or expired.')
  ...
  persistPendingOAuth(state, flowId, expiresAt = this.now() + OAUTH_PENDING_TTL_MS) {
    if (!this.isSecureStorageAvailable()) throw new Error('Secure credential storage is unavailable on this device.')
    const pending = normalizePendingOAuth({
      version: OAUTH_PENDING_VERSION,
      state,
      flowId,
    ...
    if (!pending) throw new Error('Google sign-in could not establish a valid PKCE continuation.')
    const encrypted = this.safeStorage.encryptString(JSON.stringify({ version: OAUTH_PENDING_VERSION, ...pending })).toString('base64')
    fs.writeFileSync(this.pendingOAuthPath, encrypted, { encoding: 'utf8', mode: 0o600 })
    this.pendingOAuthState = pending.state
    this.pendingOAuthFlowId = pending.flowId
    this.pendingOAuthExpiresAt = pending.expiresAt
  }
  ...
  clearPendingOAuth() {
    ...
    this.pendingOAuthExpiresAt = 0
    this.pendingOAuthStorageItems = {}
    try { fs.rmSync(this.pendingOAuthPath, { force: true }) } catch { /* best-effort cleanup */ }
  }
  ...
  readStoredSession() {
    if (!this.safeStorage?.isEncryptionAvailable?.() || !fs.existsSync(this.sessionPath)) return null
    try {
      const encrypted = Buffer.from(fs.readFileSync(this.sessionPath, 'utf8'), 'base64')
      const stored = normalizeStoredSession(JSON.parse(this.safeStorage.decryptString(encrypted)))
      if (!stored) throw new Error('Stored authentication session is malformed.')
      return stored
    } catch {
  ...
  persistSession(session, verifiedUser = null) {
    ...
    if (!this.safeStorage?.isEncryptionAvailable?.()) throw new Error('Secure credential storage is unavailable on this device.')
    const envelope = {
      version: 1,
      session,
      user: sanitizeUser(verifiedUser) ?? sanitizeUser(session.user),
    }
    const encrypted = this.safeStorage.encryptString(JSON.stringify(envelope)).toString('base64')
    fs.writeFileSync(this.sessionPath, encrypted, { encoding: 'utf8', mode: 0o600 })
  }

  clearStoredSession() {
    try { fs.rmSync(this.sessionPath, { force: true }) } catch { /* best-effort cleanup */ }
  }
  ...
  async hydrateSession() {
    ...
    try {
      const { data, error } = await this.waitForRemoteResponse(this.client.auth.setSession({
      ...
      if (error || !data?.session) {
        ...
        return { session: stored.session, user: stored.user, offline: true }
      }
      const user = sanitizeUser(data.session.user) ?? stored.user
      this.persistSession(data.session, user)
      return { session: data.session, user, offline: false }
    } catch (error) {
      if (isDefinitiveCredentialError(error)) {
        this.clearStoredSession()
        return null
  ...
  async getStatus() {
    if (!this.client) return { configured: false, authenticated: false, user: null }
    const hydrated = await this.hydrateSession()
    if (!hydrated) return { configured: true, authenticated: false, user: null }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        timeoutHandle = this.timeoutScheduler.setTimeout(() => {
          settle(reject, new Error(`Supabase Auth ${operation} timed out after ${this.hydrationTimeoutMs}ms.`))
  ...
  async beginGoogleSignIn(openExternal) {
    ...
    try {
      ...
      const redirectTo = addTimeFarmOAuthState(this.configuration.redirectUrl, state)
      ...
      if (!isValidOAuthState(state) || !isValidPkceFlowId(data.flowId)) throw new Error('Google sign-in response did not include a verifiable PKCE flow.')
  ...
  async handleOAuthCallback(callbackUrl) {
    ...
    if (!consumed) throw new Error('Google sign-in callback could not be verified.')
    try {
      const { data, error } = await this.client.auth.exchangeCodeForSession(codes[0], { flowId: consumed.flowId })
      if (error || !data?.session) throw new Error(error?.message || 'Unable to complete Google sign-in.')
      this.persistSession(data.session)
      return this.getStatus()
    } finally {
      this.pendingOAuthStorageItems = {}
  ...
  async signOut() {
    if (this.client) {
      try { await this.client.auth.signOut({ scope: 'local' }) } catch { /* local credential removal still wins */ }
    }
    this.clearStoredSession()
    this.clearPendingOAuth()
  ...
  assertConfigured() {
    if (!this.client) throw new Error('Cloud authentication has not been configured. Set TIMEFARM_SUPABASE_URL and TIMEFARM_SUPABASE_ANON_KEY.')
  }

  assertCredentials(email, password) {
    if (typeof email !== 'string' || !/^\S+@\S+\.\S+$/.test(email.trim())) throw new Error('Enter a valid email address.')
