...
test("unclaimed local data remains local and does not consult cloud authentication", async () => {
  ...
  const result = await resolveLocalAccountPrincipal({
    ...
    authService: {
      getStatus: async () => {
        statusCalls += 1;
        return { authenticated: true, user: { id: "cloud-user" } };
      },
    },
  });
  ...
  assert.deepEqual(cloudSyncEligibility(account()), {
    eligible: false,
    state: "not_claimed",
  });
});
...
test("linked data accepts its same cached owner while offline but marks it ineligible for network sync", async () => {
  ...
  assert.deepEqual(cloudSyncEligibility(account("cloud-user")), {
    eligible: true,
    state: "eligible",
  });
});
  ...
  await assert.rejects(
    () =>
      resolveLocalAccountPrincipal({
        account: account("cloud-user"),
        authService: {
          getStatus: async () => ({ authenticated: false, user: null }),
        },
      }),
    (error) =>
      error instanceof LocalAccountPrincipalError &&
      error.code === "LOCAL_ACCOUNT_OWNER_MISMATCH",
  );
  await assert.rejects(
    () =>
      resolveLocalAccountPrincipal({
