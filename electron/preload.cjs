...
contextBridge.exposeInMainWorld('worklyDesktop', {
  loadState: () => ipcRenderer.invoke('workly:load-state'),
  executeCommand: (command) => ipcRenderer.invoke('workly:execute-command', command),
  claimAuthenticatedAccount: () => ipcRenderer.invoke('workly:claim-authenticated-account'),
  bootstrapAuthenticatedAccount: () => ipcRenderer.invoke('workly:bootstrap-authenticated-account'),
  resetLocalData: () => ipcRenderer.invoke('workly:reset-local-data'),
  getSyncSummary: () => ipcRenderer.invoke('workly:get-sync-summary'),
  getTimerLeaseStatus: () => ipcRenderer.invoke('workly:get-timer-lease-status'),
  acquireTimerLease: () => ipcRenderer.invoke('workly:acquire-timer-lease'),
  getSyncConflicts: (limit) => ipcRenderer.invoke('workly:get-sync-conflicts', limit),
  resolveSyncConflict: (conflictId) => ipcRenderer.invoke('workly:resolve-sync-conflict', conflictId),
  acceptRemoteSyncConflict: (conflictId) => ipcRenderer.invoke('workly:accept-remote-sync-conflict', conflictId),
  ...
  convertMoney: (money, targetCurrency) => ipcRenderer.invoke('workly:fx-convert', money, targetCurrency),
  getOverlayPreferences: () => ipcRenderer.invoke('workly:overlay:get-preferences'),
  setOverlayPreferences: (input) => ipcRenderer.invoke('workly:overlay:set-preferences', input),
  ...
  onTimerLeaseChanged: (listener) => {
    ...
    return () => ipcRenderer.removeListener('workly:timer-lease-changed', callback)
  },
  onOverlayStopRequested: (listener) => {
    const callback = (_, value) => listener(value)
    ipcRenderer.on('workly:overlay-stop-request', callback)
    return () => ipcRenderer.removeListener('workly:overlay-stop-request', callback)
  },
})
