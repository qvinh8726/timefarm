...
create table if not exists public.profiles (
  ...
  currency text not null check (currency in ('VND', 'USD', 'EUR', 'JPY', 'GBP')),
...
create table if not exists public.projects (
  ...
  expected_currency text check (expected_currency is null or expected_currency in ('VND', 'USD', 'EUR', 'JPY', 'GBP')),
  note text,
  color text not null,
  icon text not null,
...
create table if not exists public.work_sessions (
  ...
  earnings_currency text check (earnings_currency is null or earnings_currency in ('VND', 'USD', 'EUR', 'JPY', 'GBP')),
  note text,
  remote_revision bigint not null default 1,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.fx_rate_cache (
  ...
  base_currency text not null check (base_currency in ('VND', 'USD', 'EUR', 'JPY', 'GBP')),
  quote_currency text not null check (quote_currency in ('VND', 'USD', 'EUR', 'JPY', 'GBP')),
