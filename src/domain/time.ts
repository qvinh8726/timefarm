...
export function activeDurationMs(
  ...
  if (session.status === "completed" && session.activeDurationMs !== undefined)
...
export function formatDuration(
  ...
  if (hours === 0)
    return `${minutes} ${translate(language, "duration", minutes === 1 ? "minute" : "minutes")}`;
...
export function localDayKey(
  ...
  return instantFor(value)
    .toZonedDateTimeISO(safeTimezone(timezone))
    .toPlainDate()
    .toString();
}
...
export function splitIntervalsByLocalDay(
