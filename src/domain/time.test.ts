...
describe("timer duration", () => {
  it("excludes completed and in-progress pause intervals", () => {
    ...
    expect(
      activeDurationMs(session, new Date("2026-08-09T10:30:00.000Z").getTime()),
    ).toBe(90 * 60_000);
  });
  ...
  it("creates non-overlapping active intervals", () => {
    const intervals = activeIntervals(base.startedAt, base.endedAt, [
      ...
      {
        startedAt: "2026-08-09T08:20:00.000Z",
        endedAt: "2026-08-09T08:45:00.000Z",
      },
    ]);
    expect(intervals).toEqual([
      {
        start: new Date("2026-08-09T08:00:00.000Z").getTime(),
        end: new Date("2026-08-09T08:15:00.000Z").getTime(),
      },
      {
        start: new Date("2026-08-09T08:45:00.000Z").getTime(),
        end: new Date("2026-08-09T11:00:00.000Z").getTime(),
      },
    ]);
  });

  it("allocates active time through midnight", () => {
    const split = splitIntervalsByLocalDay([
      {
        start: new Date(2026, 7, 9, 23, 30).getTime(),
        end: new Date(2026, 7, 10, 1, 0).getTime(),
      },
    ]);
  ...
  it("formats a recovery datetime-local minimum in the zone shown to the user", () => {
    expect(
      formatDateTimeLocalInput("2026-08-10T00:30:00.000Z", "America/New_York"),
    ).toBe("2026-08-09T20:30");
    expect(
      formatDateTimeLocalInput("2026-08-10T00:30:00.000Z", "Asia/Saigon"),
    ).toBe("2026-08-10T07:30");
  });
